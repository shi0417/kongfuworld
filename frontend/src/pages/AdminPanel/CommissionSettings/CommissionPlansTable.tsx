...
interface CommissionPlansTableProps {
  ...
  onPlansLoaded?: (readerPlans: any[], authorPlans: any[]) => void;
  triggerCreate?: boolean;
  onCreateTriggered?: () => void;
}

const CommissionPlansTable: React.FC<CommissionPlansTableProps> = ({
  onError,
  onPlansLoaded,
  triggerCreate,
  onCreateTriggered
  ...
  const [savingForPlans, setSavingForPlans] = useState(false);
  ...
  const loadCommissionPlans = async () => {
    try {
      ...
      if (data.success) {
        ...
        // 通过回调更新父组件的 readerPlans 和 authorPlans
        if (onPlansLoaded) {
