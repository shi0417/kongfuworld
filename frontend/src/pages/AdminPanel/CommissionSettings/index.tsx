...
const CommissionSettings: React.FC<CommissionSettingsProps> = ({ onError }) => {
  ...
  const saveRoyaltyContractEdit = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/admin/novel-royalty-contracts/${editingRoyaltyContract.id}`, {
        ...
        body: JSON.stringify({
          plan_id: editingRoyaltyContract.plan_id
        })
      });
  ...
  // 当切换到合同列表时加载数据
  useEffect(() => {
    if (commissionSettingsTab === 'author' && authorRoyaltySubTab === 'contracts') {
      loadRoyaltyContracts(royaltyContractsPage, selectedContractSearchNovelId, selectedContractSearchUserId);
    }
  }, [commissionSettingsTab, authorRoyaltySubTab, royaltyContractsPage, selectedContractSearchNovelId, selectedContractSearchUserId]);
        ...
        <div>
          ...
          {commissionSettingsTab === 'author' && (
            <button
              ...
              style={{ padding: '8px 16px', background: '#007bff', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px', fontWeight: '500' }}
            >
              新建方案
            </button>
          )}
      ...
      {commissionSettingsTab === 'author' && (
        <>
          ...
          {authorRoyaltySubTab === 'plans' && (
            <div className={styles.paymentTable}>
              <h3>作者分成方案列表 (author_royalty_plan)</h3>
            ...
            <div className={styles.paymentTable}>
              <div style={{ marginBottom: '20px', paddingBottom: '15px', borderBottom: '2px solid #e0e0e0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                  <h3 style={{ margin: 0 }}>分成方案明细 (novel_royalty_contract)</h3>
                </div>
                {/* 搜索区域 */}
        ...
        <div className={styles.modal} onClick={() => setEditingRoyaltyContract(null)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()} style={{ maxWidth: '600px' }}>
            ...
            <div className={styles.modalBody}>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold', color: '#333' }}>
                  分成方案 <span style={{ color: 'red' }}>*</span>
                </label>
                <select
              ...
              <div className={styles.modalActions} style={{ marginTop: '25px' }}>
                <button
                  ...
                  className={styles.approveButton}
                  disabled={saving || !editingRoyaltyContract.plan_id}
                  style={{ marginRight: '10px' }}
                >
                  {saving ? '保存中...' : '保存'}
                </button>
