...
async function performCheckin(userId, userTimezone = 'UTC') {
  ...
  try {
    ...
    // 更新签到任务进度
    console.log(`[DEBUG] 用户 ${userId} 签到成功，开始更新签到任务进度...`);
    
    try {
      const missionResult = await updateMissionProgress(userId, 'daily_checkin', 1);
      
      if (missionResult.success) {
        console.log(`[DEBUG] 签到任务进度更新成功:`, missionResult.data);
      } else {
        console.log(`[DEBUG] 签到任务进度更新失败:`, missionResult.message);
      }
    } catch (error) {
      console.error(`[ERROR] 更新签到任务进度失败:`, error);
    }
