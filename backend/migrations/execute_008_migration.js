/**
 * 执行迁移脚本：008_add_chief_editor_and_supervisor.sql
 * 升级 admin 表：添加主编角色和上级主管字段
 * 
 * 使用方法：
 * node backend/migrations/execute_008_migration.js
 */
...
async function executeMigration() {
  ...
  try {
    ...
    // 读取迁移SQL文件
    const sqlPath = path.join(__dirname, '008_add_chief_editor_and_supervisor.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    
    console.log('📝 开始执行迁移脚本：008_add_chief_editor_and_supervisor.sql\n');
    console.log('将执行以下操作：');
    console.log('  1. 修改 role 枚举，添加 chief_editor');
    console.log('  2. 添加 supervisor_admin_id 字段（上级主管ID）');
    console.log('  3. 添加外键约束和索引\n');
    
    // 检查字段和约束是否已存在
    console.log('🔍 检查当前状态...');
    
    // 检查 supervisor_admin_id 字段
    const [columns] = await connection.execute(
      `SELECT COLUMN_NAME 
       FROM INFORMATION_SCHEMA.COLUMNS 
       WHERE TABLE_SCHEMA = ? 
       AND TABLE_NAME = 'admin' 
       AND COLUMN_NAME = 'supervisor_admin_id'`,
      [dbConfig.database]
    );
    
    if (columns.length > 0) {
      console.log('⚠️  supervisor_admin_id 字段已存在');
    } else {
      console.log('✓ supervisor_admin_id 字段不存在，将添加');
    }
    
    // 检查 role 枚举是否包含 chief_editor
    ...
    if (roleInfo.length > 0) {
      ...
      if (columnType.includes('chief_editor')) {
        console.log('⚠️  role 枚举已包含 chief_editor');
      } else {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        // 检查是否是修改枚举的语句
        if (statement.includes('MODIFY COLUMN') && statement.includes('role')) {
          const [currentRoleInfo] = await connection.execute(
            `SELECT COLUMN_TYPE 
             FROM INFORMATION_SCHEMA.COLUMNS 
             WHERE TABLE_SCHEMA = ? 
             AND TABLE_NAME = 'admin' 
          ...
          if (currentRoleInfo.length > 0) {
            const currentType = currentRoleInfo[0].COLUMN_TYPE;
            if (currentType.includes('chief_editor')) {
              console.log(`⏭️  [${i + 1}/${statements.length}] 跳过：role 枚举已包含 chief_editor`);
              continue;
            }
          }
        }
        ...
        // 检查是否是添加字段的语句
        if (statement.includes('ADD COLUMN') && statement.includes('supervisor_admin_id')) {
          if (columns.length > 0) {
            console.log(`⏭️  [${i + 1}/${statements.length}] 跳过：supervisor_admin_id 字段已存在`);
            continue;
          }
        }
    ...
    if (constraints.length > 0) {
      console.log('\n✅ 外键约束:');
      console.log('   - fk_admin_supervisor');
    }
    ...
    const [indexes] = await connection.execute(
      `SELECT INDEX_NAME 
       ...
       WHERE TABLE_SCHEMA = ? 
       AND TABLE_NAME = 'admin' 
       AND INDEX_NAME IN ('idx_supervisor_admin_id', 'idx_role')`,
      [dbConfig.database]
    );
