/**
 * 执行迁移脚本：013_create_novel_income_monthly.sql
 * Phase 5: 创建小说月度收入表
 * 
 * 使用方法：
 * node backend/migrations/execute_013_migration.js
 */
...
async function executeMigration() {
  ...
  try {
    ...
    // 读取迁移SQL文件
    const sqlPath = path.join(__dirname, '013_create_novel_income_monthly.sql');
    const sql = fs.readFileSync(sqlPath, 'utf8');
    
    console.log('📝 开始执行迁移脚本：013_create_novel_income_monthly.sql\n');
    console.log('将执行以下操作：');
    console.log('  1. 创建 novel_income_monthly 表\n');
    ...
    const [tables] = await connection.execute(
      `SELECT TABLE_NAME 
       ...
       WHERE TABLE_SCHEMA = ? 
       AND TABLE_NAME = 'novel_income_monthly'`,
      [dbConfig.database]
    );
    
    if (tables.length > 0) {
      console.log('⚠️  novel_income_monthly 表已存在');
    } else {
      console.log('✓ novel_income_monthly 表不存在，将创建');
    }
    ...
    if (newTables.length > 0) {
      console.log('✅ novel_income_monthly 表已创建');
