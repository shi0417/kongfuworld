...
async function addReviewStatusField() {
  let db;
  try {
    console.log('开始添加review_status字段...\n');
    ...
    // 添加review_status字段
    console.log('1. 添加review_status字段到chapter表...');
    await db.execute(`
      ALTER TABLE \`chapter\` 
      ADD COLUMN \`review_status\` enum('submitted','reviewing','approved','rejected') 
      DEFAULT 'submitted' COMMENT '审核状态: submitted=提交中, reviewing=审核中, approved=审核通过, rejected=审核不通过'
    `);
    ...
    // 为现有章节设置默认审核状态
    console.log('3. 为现有章节设置默认审核状态...');
    await db.execute(`
      UPDATE \`chapter\` 
      SET \`review_status\` = 'approved' 
      WHERE \`review_status\` IS NULL
    `);
    console.log('✅ 已为现有章节设置默认审核状态为"审核通过"');
    ...
    chapters.forEach((chapter, index) => {
      ...
      console.log(`   ${index + 1}. ID: ${chapter.id}, 标题: ${chapter.title}, 审核状态: ${statusMap[chapter.review_status] || chapter.review_status}`);
    });
    ...
    statusCounts.forEach((status, index) => {
      ...
      console.log(`   ${statusMap[status.review_status] || status.review_status}: ${status.count} 个章节`);
    });
...
if (require.main === module) {
  addReviewStatusField();
}

module.exports = { addReviewStatusField };
