...
-- 1. 作者月度收入汇总表（结算层）
CREATE TABLE IF NOT EXISTS `author_income_monthly` (
  `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
  `user_id` BIGINT NOT NULL COMMENT '作者ID',
  `month` DATE NOT NULL COMMENT '月份，例如 2025-10-01 代表2025年10月',
  
  `base_income_usd` DECIMAL(10,6) NOT NULL DEFAULT 0 COMMENT '基础作者收入',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='作者月度收入汇总表';

-- 2. 作者支付单主表（提现/支付层）
CREATE TABLE IF NOT EXISTS `author_payout` (
  `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
  `user_id` BIGINT NOT NULL COMMENT '作者ID',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='作者支付单主表';

-- 3. 支付单涉及的月份
CREATE TABLE IF NOT EXISTS `author_payout_item` (
  `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
  `payout_id` BIGINT NOT NULL COMMENT '支付单ID',
  `user_id` BIGINT NOT NULL COMMENT '作者ID',
  `month` DATE NOT NULL COMMENT '对应 author_income_monthly.month',
  `amount_usd` DECIMAL(10,6) NOT NULL COMMENT '本次支付中分配到该月的金额',
  
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  
  CONSTRAINT `fk_item_payout` FOREIGN KEY (`payout_id`) REFERENCES `author_payout`(`id`) ON DELETE CASCADE,
...
-- 4. 作者收款账户配置
CREATE TABLE IF NOT EXISTS `author_payout_account` (
  `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
  `user_id` BIGINT NOT NULL COMMENT '作者ID',
  ...
  KEY `idx_user` (`user_id`),
  KEY `idx_method` (`method`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='作者收款账户配置';

-- 5. 支付网关交易表（银行/PayPal返回数据）
