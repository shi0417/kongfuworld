...
-- 1. 首页推荐小说表
-- 用于管理首页各个区块展示的小说
CREATE TABLE `homepage_featured_novels` (
...
-- 插入默认的首页配置数据
INSERT INTO `homepage_config` (`section_name`, `section_title`, `display_limit`, `sort_by`, `is_active`, `description`) VALUES
...
-- 插入默认的小说类型数据
INSERT INTO `genre` (`name`, `slug`, `chinese_name`, `is_active`) VALUES
...
-- 创建索引优化查询性能
CREATE INDEX idx_homepage_featured_novels_active_order ON homepage_featured_novels (is_active, display_order);
CREATE INDEX idx_homepage_banners_active_order ON homepage_banners (is_active, display_order);
CREATE INDEX idx_novel_statistics_date_views ON novel_statistics (date, views DESC);
CREATE INDEX idx_novel_statistics_date_reads ON novel_statistics (date, reads DESC);
